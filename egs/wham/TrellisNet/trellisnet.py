...
class TrellisNet(nn.Module):
    def __init__(self, ninp, nhid, nout, nlevels=40, kernel_size=2, dropouth=0.0,
                 wnorm=True, aux_frequency=-1, dilation=[1]):
        """
    ...
    def forward(self, X, hc, aux=False):
        ninp = self.ninp
        nout = self.nout
...
if __name__ == "__main__":
    ...
    a = TrellisNet(64, 64, 64, aux_frequency=-1).cuda()
    inp = torch.rand(2, 64 , 1000).cuda()
    hid = torch.zeros(2, 128, 1).cuda()
    print(a(inp, hid, False)[0].shape)